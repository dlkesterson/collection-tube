	...
	async function submitHandler(e) {
		...
		try {
			const res = await fetch('/api/settings', {
				...
				body: JSON.stringify({
					deleteAllRelatedDataWhenDeletingSubscription,
				}),
			});
			setSubmitting(false);
		...
		<form onSubmit={submitHandler}>
			<div className="p-4 max-w-md mx-auto bg-white rounded-xl shadow-md">
				<label className="flex items-center space-x-3">
					...
					<span className="text-gray-900 font-medium">When deleting a subscription, delete all related videos / thumbnail files</span>
				</label>
			</div>
